...
같은 requestId, 두 번 배달
  msg #1 ─┐
  msg #2 ─┴─▶ GATE 1 ──▶ GATE 2 ──▶ GATE 3 ──▶ GATE 4
  (재배달)     DB 유니크    Redis 선점   Job 상태 재확인  콜백 멱등
             (job_type,   SET NX EX    RUNNING?        초안 하나
              job_id)     모델 호출 1회  종결이면 ack
             ─────────   ──────────   ────────────    ──────────
  막는 지점:  접수 경로의   outbox 재발행 타임아웃 회수와  콜백 재시도
             중복 요청     · SQS 재배달  경합한 지연 메시지
```

**Gate 2·3 은 비용이 드는 모델 호출을 막고, Gate 1·4 는 데이터 중복을 막는다. 둘은 다른 문제다.**

Gate 3 은 이번에 워커 쪽에 새로 들어왔다. outbox 재시도와 백엔드 타임아웃 회수가 경합하면 *이미 실패로 끝난 Job* 의 메시지가 늦게 도착할 수 있다. 그 상태로 모델을 부르면 버려질 결과에 돈을 쓰는 것이다.
...
네 장치의 **순서가 어긋나면 서로를 무력화한다.** 그래서 숫자를 함께 정했다.
...
├───────────────────────────────────────┤
  redis 선점 TTL 50m (3000s)
                                        ● 50m — job timeout, 최후 회수
```

- **DLQ 도착(45m)이 Job 타임아웃(50m)보다 먼저**여야 사용자가 "워커 재시도 소진" 같은 구체적인 사유를 본다.
- **Redis 선점 TTL 이 Job 타임아웃보다 짧으면** 회수 전에 키가 풀려 재배달분이 모델을 한 번 더 부른다. 그래서 3,000초다.
