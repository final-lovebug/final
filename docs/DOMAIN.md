...
| 기준 사전집 버전 | dictionaryVersionNo | Int | O | **대조에 쓴 사전집 버전**(2026-09-15 신설, `D-93`). 발행될 때 문서 버전에 그대로 찍힌다. 이 결정 이전에 만들어진 초안만 비어 있다 |
...
- **Admin 이상(Owner 포함)만 가능한 기능**: **용어 추출**, **멤버 초대**, **워크스페이스 설정**(이름·설명 수정 등, REQ-WS-006). (**문서 삭제는 2026-09-15부터 참여자 전체로 풀렸다** — `CONFLICTS.md` `D-92`.)
...
- `requiredDocumentReviewerCount`, `requiredDictionaryReviewerCount`는 각각 **0 이상, 「참여자 수 - 1」 이하**다(현 정원 기준 **0~4**). **요청자 본인은 자기 요청을 리뷰할 수 없으므로**(`D-95`) 승인을 달 수 있는 사람은 늘 참여자 수보다 하나 적다 — 참여자 수와 같은 정족수를 허용하면 영원히 채울 수 없는 값이 저장된다. (2026-09-16 확정)
- **0이면 리뷰어 없이 바로 반영**할 수 있다. 승인·변경요청 존재 여부와 무관하게 Admin 이상이 언제든 발행할 수 있다.
- **상한(「참여자 수 - 1」 이하) 검증은 룰셋을 설정·수정하는 Workspace 쪽에서 한다.** 리뷰 요청 도메인은 룰셋 값을 그대로 신뢰하고 참여자 수를 조회하지 않는다. 참여자가 빠져 정족수를 채울 수 없게 되면 Admin 이상이 발행 조건을 우회해 처리한다. (2026-09-10 확정)
...
| 갱신 → 교정 → 리뷰 → 반영 | 그렇다 | **초안이 대조에 쓴 사전집 버전**(`D-93`) | `false` |
...
- **문서 삭제는 참여자면 누구나 할 수 있고**(`D-92`) **소프트 삭제**다. 버전 행은 함께 지우지 않는다. **파생 데이터(초안·리뷰) 정리는 삭제 이벤트를 각 도메인이 구독해 처리한다** — `document`가 초안 도메인을 참조하지 않는다.
- 제목은 **워크스페이스 안에서 중복을 허용**한다.

### 라벨 (Label)
- **라벨은 워크스페이스가 소유한다.** 이름은 워크스페이스 안에서 유일하고, **문서당 최대 5개**다.
- **라벨명은 대소문자를 구분하지 않는다**(`D-94`). 같은 워크스페이스에 대소문자만 다른 라벨은 만들어지지 않고, **표시명은 최초 생성 시 입력한 값을 유지한다** — `api`가 있는데 `API`를 붙이면 기존 `api`가 재사용된다. 비교는 앞뒤 공백을 제거한 뒤 소문자로 맞춰 한다.
- **라벨 생성은 문서 생성과 트랜잭션을 공유하지 않는다**(`D-94`). 문서 생성이 실패해도 그 요청이 만든 라벨은 워크스페이스에 남는다 — 아래 「문서에서 라벨을 떼도 라벨 자체는 남는다」와 같은 취급이다.
...
| 사전집 초안 또는 사전 개정안 | ~~문서 초안 생성~~ **막지 않는다**(2026-09-15, `D-93`), 같은 사전집에 추가 초안·개정안 |

**왜 도메인을 가로질러 막았는가** — 문서 갱신은 사전집 v_N으로 대조하는데 사전 개정이 동시에 v_N+1을 만들면, 문서 개정안이 발행될 때 「발행 시점 활성 사전집 버전」이 이미 v_N+1이어서 **대조하지 않은 버전을 기준으로 표시**하게 된다.

**그 구멍은 이제 기준 버전을 고정해 막는다**(2026-09-15 확정, `D-93`). 초안이 **대조에 쓴 사전집 버전을 들고 있고**(`DraftDocument.dictionaryVersionNo`) 발행본이 그 값을 찍으므로, 사전집이 v_N+1로 올라가면 그 문서는 정직하게 「뒤처짐」이 된다. 그래서 **사전집 초안이 진행 중이어도 문서는 갱신할 수 있다.** 사전집 개정이 며칠씩 열려 있는 동안 워크스페이스의 모든 문서가 갱신을 못 하는 편이 더 큰 손해다.

**반대 방향은 그대로다** — 문서 초안·개정안이 진행 중이면 사전집 초안(용어 추출)을 시작할 수 없다. 추출은 정렬된 문서만 대상으로 하는데(`G-12`) 갱신 중인 문서의 본문이 곧 바뀌기 때문이다.
...
- **초안은 대조에 쓴 사전집 버전을 들고 있고, 발행본이 그 값을 찍는다**(2026-09-15 확정, `D-93`). 그래서 **사전집 초안이 진행 중이어도 문서 갱신을 막지 않는다.** 교정하는 동안 사전집이 다음 버전으로 올라가면 반영된 문서는 「뒤처짐」이 되고, 사용자가 다시 갱신하면 된다.
