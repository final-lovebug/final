...
| 권한 | permission | Enum | O | 소유자(Owner) / 관리자(Admin) / 멤버(Member) |
...
| 부여 권한 | permission | Enum | O | 수락 시 부여할 권한. 관리자(Admin) / 멤버(Member) |
...
- 설정 변경은 아래에 따로 적지 않은 한 **Admin 이상**만 가능하다. 조회는 참여자면 누구나 가능하다.
...
- 초대로 줄 수 있는 권한은 **Admin / Member**다. Owner는 초대로 부여하지 않는다.

**삭제**
- **Owner는 삭제 대상이 아니다.** Admin은 **Member만** 내보낼 수 있고, Admin을 내보내는 것은 Owner만 가능하다.
...
> - **참여자 권한 변경 주체** — Owner 전용으로 둘지, Admin에게도 Member 승격·강등을 허용할지
