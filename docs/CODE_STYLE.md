...
허용한다 — `@Getter`, `@RequiredArgsConstructor`, `@NoArgsConstructor(access = AccessLevel.PROTECTED)`, `@Slf4j`, 그리고 테스트 fixture 중심으로 제한적으로 쓰는 `@Builder`.

지양한다. 아래의 "Entity"는 domain 패키지의 도메인 모델을 가리킨다. 도메인 모델이 JPA 엔티티를 겸하므로 두 규칙이 같은 클래스에 적용된다.

- Entity의 무분별한 `@Setter`, Entity의 `@Data`
