...
> **실행 상태 — 일부만 실행할 수 있다.**
> `WLSH-164`(트랙 A)로 **프런트 실 연동이 절반쯤 들어왔다**(`docs/task/fe-be-1st-report.md`). 인증·워크스페이스·문서·사전집 조회는 진짜 백엔드를 치고, 사전 초안(후보어)·문서 초안·리뷰·알림은 아직 목데이터다.
> **검증면이 `화면`인 행 중 목업 화면에 걸린 것은 실행하지 않는다** — 목데이터를 상대로 통과시켜도 아무것도 증명하지 못한다. 어느 화면이 실 연동인지는 **「6-1. 프런트 연동 현황」**에 도메인별로 정리했고, 아직 실행할 수 없는 행은 `비고`에 **⛔ 목업**으로 표시했다.
...
프런트가 다른 호스트·포트의 백엔드를 봐야 하면 `frontend/.env.local.example`을 `.env.local`로 복사해 `VITE_API_BASE_URL`을 바꾼다. 로컬 기본 구성(백엔드 `:8080`)이면 파일 없이도 동작한다.
...
> **`S-6`~`S-9`는 지금 화면으로 끝까지 몰 수 없다.** 사전 초안(후보어)·문서 초안·리뷰 화면이 아직 목데이터라(「6-1」), 그 구간은 Swagger로 진행한 뒤 결과만 화면에서 확인한다. `S-1`·`S-4`·`S-5`와 사전집 조회는 실 연동이라 화면으로 그대로 된다.
...
| QA-DOC-009-1 | REQ-DOC-009 | 새 라벨 이름을 넣으면 워크스페이스 라벨로 생긴다. | 화면 | `.../settings/labels` → 「새 라벨 이름」에 입력 후 추가 | 목록에 새 라벨이 나타난다 | | ⛔ 목업 — 연동 전까지 실행하지 않는다 |
| QA-DOC-009-2 | REQ-DOC-009 | 같은 이름의 라벨은 두 번 생기지 않는다. | 화면 | 방금 만든 라벨과 같은 이름을 다시 추가 | 중복 안내가 뜨거나 항목이 늘지 않는다 | | ⛔ 목업 — DB에 `(workspace_id, name)` 유니크 |
...
| QA ID | 요구사항 | 시나리오 | 검증면 | 절차 | 기대 결과 | 결과 | 비고 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| QA-EXT-008-1 | REQ-EXT-008 | 추출을 요청하면 기다리지 않고 접수된다. | 화면 | `S-5` 이후 `.../documents/{documentId}/extract`에서 추출 실행 | 화면이 멈추지 않고 진행 상태 표시로 넘어간다 | | ⛔ 목업 — 서버는 `202` + `jobId` |
...
| QA ID | 요구사항 | 시나리오 | 검증면 | 절차 | 기대 결과 | 결과 | 비고 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| QA-REV-002-1 | REQ-REV-002 | 후보 단어를 고르면 근거 문장과 출현 문서가 함께 보인다. | 화면 | `S-7`에서 후보 단어를 추가한 뒤 그 항목 선택 | 「근거 문장」과 출현/문서 수가 표시된다 | | ⛔ 목업 — 추가할 때 근거 문장을 넣어야 보인다 |
...
| QA-REV-003-1 | REQ-REV-003 | 표준어 후보를 고쳐 저장하면 목록에 반영된다. | 화면 | `S-7`의 초안 화면에서 「표준어 후보」를 수정 후 저장 | 목록의 표기가 바뀐다 | | ⛔ 목업 — 연동 전까지 실행하지 않는다 |
...
| QA-REV-004-1 | REQ-REV-004 | 정의를 편집해 저장할 수 있다. | 화면 | 초안 화면에서 「정의」를 고쳐 저장 | 고친 정의가 유지된다 | | ⛔ 목업 — 연동 전까지 실행하지 않는다 |
...
| QA-REV-005-1 | REQ-REV-005 | 후보를 등재 승인하면 상태가 바뀐다. | 화면 | `S-7`에서 후보를 승인 | 해당 항목의 상태 표시가 승인으로 바뀐다 | | ⛔ 목업 — 연동 전까지 실행하지 않는다 |
...
| QA-DIC-004-1 | REQ-DIC-004 | 초안에서 후보 단어를 직접 추가할 수 있다. | 화면 | `.../dictionary/draft` → 「새 후보 단어」에 입력 후 추가 | 목록에 추가된다 | | ⛔ 목업 — AI 추출과 무관한 수동 등록 경로 |
| QA-DIC-004-2 | REQ-DIC-004 | 초안에서 이전 사전집 용어도 고칠 수 있다. | 화면 | 사전집 v1 발행 후 두 번째 추출을 돌려 초안에 들어온 기존 용어를 수정 | 수정한 내용이 유지된다 | | ⛔ 목업 — 확정 사전집을 직접 고치는 경로는 없다 |
...
| QA-DIC-006-1 | REQ-DIC-006 | 버전 이력이 시간순으로 보인다. | 화면 | `.../dictionary/history` | 발행된 버전이 순서대로 보인다 | | ⛔ 목업 — 연동 전까지 실행하지 않는다 |
...
| QA ID | 요구사항 | 시나리오 | 검증면 | 절차 | 기대 결과 | 결과 | 비고 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| QA-CHK-NFR001-1 | NFR-CHK-001 | 대조를 요청하면 기다리지 않고 접수된다. | 화면 | `S-8` 이후 문서 상세에서 대조(재검사) 실행 | 화면이 멈추지 않고 진행 상태로 넘어간다 | | ⛔ 목업 — 서버는 `202` + `jobId` |
...
| QA ID | 요구사항 | 시나리오 | 검증면 | 절차 | 기대 결과 | 결과 | 비고 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| QA-UPD-001-1 | REQ-UPD-001 | 제안을 적용하면 미리보기 본문에 대체어가 들어간다. | 화면 | `S-9`의 검토 화면에서 제안을 「적용」 → 미리보기 확인 | 원래 표기가 대표어로 바뀌어 보인다 | | ⛔ 목업 — **스텁이라 제안이 0건이다.** 손으로 제안을 추가한 뒤 실행한다 |
| QA-UPD-001-2 | REQ-UPD-001 | 처리하지 않은 제안이 남으면 검토를 끝낼 수 없다. | 화면 | 제안을 남긴 채 「검토 완료 · 리뷰 요청」 | 미처리 제안이 있다는 안내가 뜬다 | | ⛔ 목업 — 연동 전까지 실행하지 않는다 |
...
| QA-UPD-004-1 | REQ-UPD-004 | 정족수를 채우면 반영할 수 있다. | 화면 | `S-8`에서 B가 승인한 뒤 A가 반영 | 새 버전이 발행된다 | | ⛔ 목업 — `S-4`의 정족수 1 전제 |
...
- **프런트 연동이 절반이다.** 검증면이 `화면`인 행 중 **⛔ 목업**이 붙은 것은 실행하지 않는다. 도메인별 현황은 아래 「6-1」에 있다.
...
- **알림 패널은 껍데기만 있다.** `NotificationPanel`이 들어왔지만 `features/notification/api`가 전부 목데이터라 백엔드 알림이 화면에 오지 않는다. `REQ-NTF-004`·`008`은 계속 `API`로 검증한다(`T-INT-13`은 `F` 사유로 보류 중).
- **화면이 없어 `API`로 검증하는 기능들** — 워크스페이스 생성·이름 수정·삭제, 멤버 초대·역할 변경·제외, 문서 삭제·수정, 후보어 일괄 판정, 알림. 화면이 생기면 「7」의 2·3번에 따라 옮긴다.
- **`DocumentReviewPage` 본문이 특정 문서 2건에 하드코딩돼 있다.** `docs/task/fe-be-1st-report.md`의 문제 **D**로 재작업 대기다. `QA-UPD-001-*`의 화면 절차는 그 재작업 뒤에야 의미가 있다.
...
`WLSH-164` 트랙 A 기준이다(`docs/task/fe-be-1st-report.md`). **`features/{domain}/api`가 `httpClient`를 쓰면 실 연동, `delay()`로 픽스처를 돌려주면 목업**이다. 화면이 아니라 이 api 계층이 기준이라는 점에 주의한다 — 화면은 둘 다 똑같이 그려진다.

| 도메인 | 상태 | 실 연동된 호출 | 아직 목업인 호출 |
| --- | --- | --- | --- |
| auth | ✅ 전부 | 교환·닉네임 등록·로그아웃 | — |
| member | ✅ 전부 | 내 정보·탈퇴·참여자 목록(`T-INT-14`·`T-INT-18`) | — |
| workspace | ✅ 전부 | 목록·룰셋 조회·룰셋 수정(`T-INT-9`) | — |
| document | 🟡 절반 | 생성·목록·상세·버전 이력·라벨 목록(`T-INT-10` 5/10) | 라벨 생성, 문서 초안 목록, 제안 조회·처리·이력 |
| dictionary | 🟡 절반 | 사전집 조회, 추출 대상 문서 목록(`T-INT-11` 축소) | 후보어 목록·추가·수정, 개정 이력 타임라인 |
| review | 🔴 전부 목업 | — | 리뷰 요청 목록·생성, 코멘트, 개정 diff (`T-INT-12` 프런트 대기) |
| notification | 🔴 전부 목업 | — | 목록·미읽음 수·읽음 처리·설정 (`T-INT-13` 보류) |

이 표가 바뀌면 「7」의 3번에 따라 해당 행의 `비고`에서 **⛔ 목업**을 떼고 절차를 화면 조작으로 바꾼다.
