...
- **같은 프로세스 안에서만 이어져야 하는 신호는 도메인 이벤트 버스로 보내지 않는다.** `@TransactionalEventListener`는 스프링 `ApplicationEvent`만 받으므로, 버스가 외부 큐 어댑터로 갈린 환경에서는 그 리스너가 아예 호출되지 않는다. 내부 신호는 항상 존재하는 인프로세스 발행기로 보낸다.
...
**커밋 뒤 리스너가 외부로 무언가를 발행한다면 그 실패는 반드시 드러나야 한다.** 메시지 발행 실패는 ERROR로 남기고(`docs/LOG.md`), 그 발행에 걸려 있던 작업은 실패로 끝내 재요청이 가능한 상태로 되돌린다 — 진행 중인 채로 두면 중복 방지 정책이 다음 요청을 막는다.
...
- **메시지에 본문을 싣지 않는다.** 식별자만 보내고 큰 값은 워커가 다시 읽는다. 크기 한도를 피하는 것이 첫 이유지만, 발행 트랜잭션이 롤백돼 유령 메시지가 남아도 아무것도 오염시키지 않는다는 것이 더 중요하다.
- **외부가 결과를 돌려주는 인바운드 콜백도 presentation 규칙을 그대로 따른다.** 요청 DTO → command 변환, 도메인 모델 미노출, 트랜잭션 경계는 service. 인증 주체가 없다는 것만 다르다.
- **콜백 핸들러는 멱등하고, 종단 상태에서는 아무 일도 하지 않으며, 그 경우에도 2xx로 답한다.** 4xx로 답하면 외부가 영원히 재시도한다.
- **응답 코드가 외부의 재시도 정책을 정한다.** 4xx는 재시도 금지, 5xx와 타임아웃만 재시도다. 이 표는 계약 문서에 적고 **컨트롤러 테스트가 상태 코드를 못 박는다** — 표만 있고 테스트가 없으면 코드가 조용히 어긋난다.
- **외부가 끝내 응답하지 않는 경우를 설계에 넣는다.** 작업 행이 진행 중으로 남으면 중복 방지 정책이 다음 요청을 영구히 막으므로, 제한 시간이 지난 작업을 회수하는 경로가 함께 있어야 한다.
- **외부 큐 발행은 transactional outbox로 한다.** Job과 요청 payload를 같은 DB 트랜잭션에 저장하고, 별도 디스패처가 재시도한다. 워커 재시도 소진은 요청 DLQ를 Spring이 소비해 Job `FAILED`로 보상한다. DLQ는 발행 실패를 복구하지 못하므로 outbox를 대체하지 않는다.
