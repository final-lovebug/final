...
| 문서 버전 | DocumentVersion | 문서가 확정된 시점의 불변 스냅샷 | 개정판, 릴리스 |
| 본문 | Content | 문서의 실제 텍스트 | 내용, 콘텐츠 |
...
| 사전집 대조 | DictionaryContrast | 문서 본문을 사전집과 비교해 비표준 표현을 찾는 행위 | 대조(Collation), 비교(Comparison), 검출(Detection) |
