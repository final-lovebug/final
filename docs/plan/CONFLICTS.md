...
| ID | 확정 내용 | 해소 |
| --- | --- | --- |
| **D-111** | **호출 멱등은 워커의 책임이다.** `requestId`가 멱등 키이며(`D-70`), 워커는 **모델을 부르기 전에** 이 키를 선점하고 선점에 실패하면 모델을 부르지 않고 메시지를 지운다. 키 수명은 `app.ai.timeout.job`(기본 `PT15M`) 이상이고 저장소는 워커 인스턴스 사이에 공유돼야 한다. 규격은 `docs/AI_CONTRACT.md` 7-2-2 | `D-72`가 「결과 멱등」과 「호출 멱등」을 함께 뜻하는 것처럼 읽혀, 계약서가 워커에게 **모델 재호출도 안전하다**고 말하고 있었다 |
