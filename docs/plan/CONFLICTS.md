...
| `D-95`~`D-98` | **사전집 개정안 리뷰 UI 수정 세션(2026-09-15)이 확정한 결정** | 전역 | 이 문서 3-13절 |
| `D-99`~`D-105` | **관측성 도입 세션(2026-09-15)이 확정한 결정** | 전역 | 이 문서 3-14절 |
| `D-106`~`D-109` | **Flyway 재도입·스키마 정합 세션(2026-09-16)이 확정한 결정** | 전역 | 이 문서 3-15절 |
| `D-110` | **룰셋 정족수 상한 정합 세션(2026-09-16)이 확정한 결정** | 전역 | 이 문서 3-16절 |
| `D-111`~`D-113` | **AI 작업 동시성 세션(2026-09-16)이 확정한 결정** | 전역 | 이 문서 3-17절 |
...
| `R-32`~`R-33` | **문서 갱신 상호 배타 완화가 뒤집은 결정** | 전역 | 이 문서 4-6절 |
| `R-34` | **룰셋 정족수 상한 정합이 뒤집은 결정** | 전역 | 이 문서 4-7절 |
| `R-35` | **호출 멱등(워커 선점) 제거가 뒤집은 결정** | 전역 | 이 문서 4-8절 |
...
| **G-7** | **문서 갱신** = 대조 → 초안 → 리뷰어 지정 + 리뷰 요청 → 개정안 → 승인 → 새 버전. 새 버전의 기준 사전집 버전은 ~~**발행 시점 최신 사전집 버전**~~ **초안이 대조에 쓴 사전집 버전**을 따른다(`R-33`) | 확정. 기준 버전만 `D-93`이 바꿨다 |
...
| ID | 확정 내용 | 상태 |
| --- | --- | --- |
| **G-14** | **워크스페이스 단위 상호 배타.** 문서 초안/개정안이 존재하면 사전집 초안을 만들 수 없고, ~~사전집 초안/개정안이 존재하면 문서 초안을 만들 수 없다~~(`R-32`로 해제). 같은 대상에 추가 초안/개정안도 금지 | **한쪽만 유효**(`D-93`) |
| **G-15** | 사전 초안이 진행 중이면 그 초안의 **원천 문서(`sourceDocumentIds`)를 직접 편집할 수 없다** | 확정 |

**G-14가 왜 필요했는지** — 문서 갱신은 사전집 v_N으로 대조하는데 사전 개정이 동시에 v_N+1을 만들면, 문서 개정안이 발행될 때(`G-7`) "최신 사전집 버전"이 이미 v_N+1이어서 **대조하지 않은 버전을 기준으로 표시**하게 된다. 이 제약이 그 구멍을 막고 있었다.

> **2026-09-15 — 구멍을 다른 방법으로 막고 한쪽 방향을 열었다**(`D-93`). 기준 버전을 발행 시점에 조회하는 대신 **대조 시점에 초안에 얼려 두면** 같은 구멍이 닫힌다. 그래서 「사전집 초안이 있으면 문서 초안을 만들 수 없다」는 해제했고(`R-32`), 반대 방향(문서 초안이 있으면 사전집 초안을 만들 수 없다)은 그대로 둔다.
...
## 3-10. 문서 삭제 권한 완화 세션이 확정한 결정 (`D-92`)

2026-09-15. 사용자가 「문서 삭제는 워크스페이스 모든 권한이 할 수 있게」로 정책을 바꿨다. 백엔드에만 있고 화면에는 없던 삭제 기능을 드러내는 것도 이 세션이다.

| ID | 확정 내용 | 해소 |
| --- | --- | --- |
| **D-92** | **문서 삭제는 참여자면 누구나 할 수 있다.** `DocumentService.delete`가 `validateAtLeast(..., Permission.ADMIN)` 대신 `validateParticipant`를 쓴다. 근거 둘 — ① 소프트 삭제라 문서 행과 버전 행이 남아(`DocumentRemover`) 되돌릴 여지가 있다 ② `DOMAIN.md`가 이미 「Regular는 문서 작성·교정·리뷰까지 가능」으로 쓰기 권한을 열어 둔 마당에, 삭제만 막아 Regular가 자기 문서를 정리하려면 Admin을 불러야 했다. **비참여자는 그대로 `404 WORKSPACE_NOT_FOUND`**다 — 존재를 숨기는 규칙은 건드리지 않는다. 화면은 **문서 상세 툴바 오른쪽 끝**에 삭제 버튼을 둔다 — 목록은 행·카드 전체가 상세로 가는 클릭 영역이라 버튼을 넣으면 두 뷰 각각에 예외를 만들어야 하고, 되돌리기 어려운 동작은 내용을 확인한 자리에서 누르는 편이 안전하다 | `R-31` 발생 |

> **용어 추출·멤버 초대·워크스페이스 설정은 그대로 ADMIN 이상이다.** 이번 완화는 문서 삭제 하나에만 적용된다.

---

## 3-11. 문서 갱신 상호 배타 완화 세션이 확정한 결정 (`D-93`)

2026-09-15. 사용자가 「사전집 초안이 있어도 문서를 최신 사전집으로 갱신할 수 있게」로 정책을 바꿨다. 사전집 개정은 며칠씩 열려 있는데 그동안 워크스페이스의 모든 문서가 갱신을 못 하는 것이 이유다.

| ID | 확정 내용 | 해소 |
| --- | --- | --- |
| **D-93** | **사전집 초안이 진행 중이어도 문서 갱신(대조·초안 생성)을 막지 않는다.** `G-14`가 이 방향을 막던 이유는 개정본이 **대조한 적 없는 사전집 버전을 기준으로 찍히는 것** 하나였고(`G-7`), 그 구멍은 **대조에 쓴 사전집 버전을 초안에 고정**해 닫는다 — `DraftDocument.dictionaryVersionNo`(신규 컬럼, `V430`)에 대조 완료 시점의 활성 버전을 적고 `ReviseProcessor`가 발행 시점 활성 버전 대신 그 값을 넘긴다(`R-33`). 사전집이 v_N+1로 올라가면 그 문서는 **정직하게 「뒤처짐」이 된다** — 예전에는 대조하지 않은 v_N+1을 기준으로 「최신」이라 표시됐을 자리다. **반대 방향은 그대로 유지한다** — 문서 초안이 진행 중이면 사전집 초안·용어 추출은 여전히 막힌다(`DraftDictionaryCreationPolicyValidator`). 이 결정 이전에 만들어진 초안만 기준 버전이 `null`이며 종전대로 발행 시점 활성 버전을 따른다 | `G-14`의 한 방향, `G-7` |

> **기준 버전을 「대조 접수」가 아니라 「대조 완료」 시점에 읽는다.** 용어를 읽는 주체가 백엔드가 아니라 FastAPI 워커라서(`docs/AI_CONTRACT.md`) 워커가 실제로 읽은 버전을 콜백이 알려 주지 않는다. 완료 시점이 그 순간에 가장 가깝고, 남는 창(워커의 조회 ~ 콜백 도착)은 초 단위다. 워커가 사전집 버전을 콜백에 실어 주게 되면 그 값을 쓰는 것이 정확하다.
...
## 3-13. 사전집 개정안 리뷰 UI 수정 세션이 확정한 결정 (`D-95`~`D-98`)

2026-09-15. 개정안 리뷰 목업과 실제 화면을 대조해, 이미 백엔드에 있던 최신 리뷰 집계 규칙과 프런트 표시 규칙을 일치시켰다.

| ID | 확정 내용 | 해소 |
| --- | --- | --- |
| **D-95** | **요청자 본인은 리뷰를 제출할 수 없다.** 참여자 검증 뒤 `REVIEW_REQUEST_SELF_REVIEW_NOT_ALLOWED`(403)로 차단하며, 요청자를 리뷰어로 지정하는 경로도 같은 검증을 쓴다 | 리뷰 제출·리뷰어 지정 권한 정합 |
| **D-96** | **리뷰 목록은 회차와 무관하게 회원별 최신 1건을 중심으로 표시한다.** 이전 리뷰는 이력으로 접어 두고 회차 배지는 화면에 표시하지 않는다 | `D-1`과 화면 정합 |
| **D-97** | **리뷰 섹션은 용어 표 아래에 사람별 묶음으로 두고, 우측은 선택 용어 코멘트를 상시 표시한다.** 전체 코멘트(`targetItemId`·`anchor` 없음)와 용어 코멘트를 모두 노출한다 | 목업 구조 반영 |
| **D-98** | **답글 UI는 만들지 않는다.** 기존 `parentId`·`children` 데이터는 평탄화해 읽고, 코멘트 없는 리뷰도 제출·표시한다 | `Comment` 스키마 보존, UI만 평면화 |

> **원래 `D-92`~`D-95`로 배정됐던 결정이다.** `feat/WLSH-178-document-fix`가 같은 번호를 다른 결정에 먼저 쓰고 있어 병합하면서 옮겼다 — 이 번호들은 `CONFLICTS.md` 밖에서 인용된 적이 없어 옮기는 비용이 없었다(문서·코드 참조 0건).

---

## 3-14. 관측성 도입 세션이 확정한 결정 (`D-99`~`D-105`)

2026-09-15. 관측성 도입에서 확정한 내용을 기존 결정 번호와 충돌하지 않도록 `D-99`부터 배정한다.

| ID | 확정 내용 | 해소 |
| --- | --- | --- |
| **D-99** | 앱이 Grafana Cloud로 OTLP를 직접 보낸다. 수집기 사이드카는 두지 않는다 | 런타임 전송 단절 |
| **D-100** | 트레이스·메트릭·로그를 모두 전송하고 앱 로그는 stdout과 OTLP로 남긴다. 파일 appender는 두지 않는다 | 재배포 시 로그 유실 |
| **D-101** | Grafana Cloud 전송은 `prod`에서만 사용하고 로컬·테스트는 LGTM을 사용한다 | 환경별 관측 경로 혼용 |
| **D-102** | Grafana Cloud 자격증명은 Parameter Store `/lovebug/otel/` 이하에서 주입한다 | 배포 설정의 시크릿 노출 |
| **D-103** | `traceId`는 Micrometer Tracing이 MDC에 넣은 값을 `ErrorResponse`에도 사용한다 | 응답·로그·트레이스 상관관계 단절 |
| **D-104** | actuator는 `health`·`info`만 무인증 공개하고 나머지는 인증하며 `env`·`configprops`는 영구 제외한다 | actuator 전체 공개 |
| **D-105** | 관측성 exporter 장애가 애플리케이션 가용성을 낮추지 않도록 구성하고 exporter 자체 로그의 되먹임을 차단한다 | exporter 장애 전파 |

`NFR-INF-008`의 Spring → SQS → FastAPI `traceparent` 전파와 `NFR-CMN-001`의 실제 p95 측정은 이 세션의 미완료 범위로 남긴다.

---

## 3-15. Flyway 재도입·스키마 정합 세션이 확정한 결정 (`D-106`~`D-109`)

2026-09-16. 스키마 검증 실패로 `local`·`dev`·배포에서 Flyway를 끈 상태(`ec0270d`·`add9940`)를
되돌리며 확정했다. 실행 체크리스트는 `docs/task/T-INT-24-flyway-schema-alignment.md`다.

| ID | 확정 내용 | 해소 |
| --- | --- | --- |
| **D-106** | 스키마의 주인은 Flyway 하나다. 마이그레이션과 엔티티가 어긋나면 **엔티티를 고친다** | 스키마 정의가 두 벌로 갈림 |
| **D-107** | `ddl-auto`는 모든 프로파일에서 `validate`다. 어느 환경에서도 엔티티가 스키마를 만들지 않는다 | `prod`가 `none`이라 검증조차 하지 않음 |
| **D-108** | MySQL 컨테이너 + Flyway + `validate`로 컨텍스트만 띄우는 테스트를 `check`에 상주시킨다 | 드리프트의 첫 검출 지점이 기동 시점이던 문제 |
| **D-109** | MySQL 타입 대응 규칙(`@Lob` 금지 등)을 `backend/CLAUDE.md`에 둔다 | 같은 종류의 불일치가 컬럼마다 재발 |

`validate`는 컬럼의 존재와 타입만 본다. `nullable`·기본값·인덱스·외래키·collation은 검증 범위 밖이며
마이그레이션 리뷰가 유일한 방어선이다. **`prod`의 Flyway 재개는 운영 `flyway_schema_history` 정리
판단이 필요해 이 세션의 미완료 범위로 남긴다.**

---

## 3-16. 룰셋 정족수 상한 정합 세션이 확정한 결정 (`D-110`)

2026-09-16. 워크스페이스 설정의 승인 규칙 화면에서 스탭퍼·저장 버튼이 눌리지 않는다는 제보에서 출발했다.
화면은 정상이었고(상한을 「참여자 수 - 1」로 잡고 있었다) **백엔드 검증만 한 칸 느슨했다.**

| ID | 확정 내용 | 해소 |
| --- | --- | --- |
| **D-110** | **룰셋 정족수의 상한은 「참여자 수 - 1」이다.** 요청자 본인은 자기 요청을 리뷰할 수 없으므로(`D-95`) 승인을 달 수 있는 사람은 늘 참여자 수보다 하나 적다. `RuleSet.exceedsParticipantCount` → `exceedsReviewerCapacity`로 바꾸고 `RuleSetValidator`가 이 값을 쓴다 | 참여자 수와 같은 정족수를 저장할 수 있었고, 그 값은 **영원히 채울 수 없어** Admin의 발행 우회 없이는 교착이었다 |

**참여자가 요청자 혼자인 워크스페이스에서는 정족수가 `0`으로 고정된다** — 상한이 0이라 스탭퍼가 움직이지
않는 것이 정상 동작이다. 정족수를 올리려면 멤버를 먼저 초대해야 한다.

참여자가 빠져 이미 저장된 정족수가 상한을 넘게 되는 경우는 **종전대로 막지 않는다**(`DOMAIN.md` 룰셋 규칙,
2026-09-10 확정) — 소급 조정 없이 Admin 이상의 발행으로 푼다.

---

## 3-17. AI 작업 동시성 세션이 확정한 결정 (`D-111`~`D-113`)

2026-09-16. 「용어 추출·문서 대조를 동시에 누르면 LLM 이 여러 번 불리는가」라는 물음에서 출발했다.
**불린다.** 경로가 둘이고 성격이 다르다.

| 경로 | 무엇이 일어나는가 | 지금까지 막던 것 |
| --- | --- | --- |
| **작업이 둘 생긴다** | 생성 정책 검사(`ExtractionJobCreationPolicyValidator`·`CheckJobCreationPolicyValidator`)가 락 없는 스냅샷 읽기라 동시 요청 둘을 모두 통과시킨다. 작업이 둘이면 `requestId`도 둘이라 워커는 둘 다 정상 작업으로 보고 모델을 두 번 부른다 | **없었다.** `D-72`의 콜백 멱등은 *같은* 작업의 중복 콜백만 막는다 |
| **같은 메시지가 두 번 배달된다** | 요청 큐는 표준 큐이고 at-least-once 다(`D-53`). 워커가 같은 `requestId` 로 모델을 두 번 부르고 콜백을 두 번 보낸다 | `D-72`가 **초안만** 하나로 접는다. 모델 호출은 이미 끝났고 요금도 나갔다 |

| ID | 확정 내용 | 해소 |
| --- | --- | --- |
| ~~**D-111**~~ **(`R-35`로 뒤집힘)** | **호출 멱등은 워커의 책임이다.** `requestId`가 멱등 키이며(`D-70`), 워커는 **모델을 부르기 전에** 이 키를 선점하고 선점에 실패하면 모델을 부르지 않고 메시지를 지운다. 키 수명은 `app.ai.timeout.job`(기본 `PT15M`) 이상이고 저장소는 워커 인스턴스 사이에 공유돼야 한다. 규격은 `docs/AI_CONTRACT.md` 7-2-2 | `D-72`가 「결과 멱등」과 「호출 멱등」을 함께 뜻하는 것처럼 읽혀, 계약서가 워커에게 **모델 재호출도 안전하다**고 말하고 있었다 |
| **D-112** | **「워크스페이스당 진행 중 추출 작업 1개」·「문서당 진행 중 대조 작업 1개」의 최종 판정자는 DB의 유니크 제약이다.** `extraction_job`·`check_job`에 `in_progress_flag`(진행 중이면 1, 끝나면 `NULL`)를 두고 각각 `(workspace_id, in_progress_flag)`·`(document_id, in_progress_flag)`에 유니크를 건다. 사전 검사는 그대로 두되 제약 위반은 writer 가 기존 409로 바꿔 던진다 | 사전 검사만으로는 동시 요청 둘을 모두 통과시켜 작업이 둘 생겼다 |
| ~~**D-113**~~ **(`R-35`로 뒤집힘)** | **`D-111`의 선점 저장소는 Redis 다.** 워커가 모델 호출 직전에 `SET llm:request:{requestId} <worker-id> NX EX 900`으로 선점하고, 실패하면 모델을 부르지 않고 메시지를 지운다. **키는 지우지 않고 TTL 만료에 맡긴다** — 지우면 그 뒤 재배달분이 다시 선점에 성공한다. **선점은 `mode=REAL`에만 한다** — `STUB`·`MOCK`은 모델을 부르지 않으므로, 목 대역만 쓰는 로컬 개발이 Redis 없이 돌아야 한다. `REDIS_URL`이 없거나 Redis 가 죽으면 그 `REAL` 작업을 **실패로 끝낸다**(선점 여부를 모른 채 부르면 막으려던 일이 그대로 일어난다). 규격은 `docs/AI_CONTRACT.md` 7-2-2 | 백엔드 claim 엔드포인트 안을 물리친 이유는 **인증 없는 `/api/internal/**`에 상태를 바꾸는 경로를 하나 더 여는 것**이기 때문이다(`D-70`의 방어가 `requestId` 대조뿐인 상황) |

**`D-112`의 플래그는 `dictionary.active_flag`와 달리 생성 컬럼이 아니다.** 같은 식을 생성 컬럼으로 두면
H2 위에서 도는 테스트가 **삽입 시점에** `The database has been closed`(H2 90098)로 깨진다 — 같은 DDL·같은
H2 버전·같은 INSERT 를 순수 JDBC 로 재현하면 정상 동작하므로 Hibernate 경로에 얽힌 문제로 보이나 원인은
규명하지 못했다. 값은 `ExtractionJob.changeStatus`·`CheckJob.changeStatus`가 상태와 함께 채운다 —
상태를 바꾸는 경로가 그 메서드 하나뿐이라 어긋날 지점이 없다.

**잔여 과제 둘은 이 세션의 범위 밖이다.**

- `markDispatching`(`PENDING → RUNNING`)이 여전히 read-check-write 다. 조건부 `UPDATE`로 바꾸면 발행 측 중복이 원자적으로 막힌다
- 같은 작업에 콜백이 동시에 두 번 들어오면 `draft_dictionary`·`draft_document`가 둘 생길 수 있다. `D-112`와 같은 패턴으로 닫을 수 있다

---

## 4. 뒤집힌 기존 결정 (`R-1`~`R-34`)
...
### 4-5. 문서 삭제 권한 완화가 뒤집은 결정 (`R-31`)

| ID | 뒤집힌 결정 | 출처 | 대체 |
| --- | --- | --- | --- |
| **R-31** | 「문서 삭제는 **Admin 이상**」 | `DOMAIN.md` «워크스페이스 · 권한»·문서 정책 / `API.md` «문서 삭제»와 엔드포인트 표 / `REQ-DOC-004` / `WORKSPACE_PLAN.md` 9절 권한 표 / `DOCUMENT_PLAN.md` 4·6절(as-built 표·권한 표·「삭제만 ADMIN 이상」) / `DRAFT_DOCUMENT_PLAN.md` 9절 권한 표 | **참여자면 누구나 삭제한다**(`D-92`). `DOMAIN.md`·`API.md`·`REQUIREMENTS.md`는 고쳤고, **`docs/plan/`의 세 문서는 보존 원칙에 따라 고치지 않는다** — 그 서술과 어긋나면 이 `R-31`을 따른다 |

### 4-6. 문서 갱신 상호 배타 완화가 뒤집은 결정 (`R-32`~`R-33`)

| ID | 뒤집힌 결정 | 출처 | 대체 |
| --- | --- | --- | --- |
| **R-32** | `G-14`의 한 방향 — 「사전집 초안/개정안이 존재하면 **문서 초안**을 만들 수 없다」 | 이 문서 2-3절 / `DOMAIN.md` «초안 — 워크스페이스 단위 상호 배타» 표 / `API.md` 오류 표 「같은 문서 **또는 워크스페이스**에 진행 중인 초안이 있음」 / `DRAFT_DOCUMENT_PLAN.md` | **막지 않는다**(`D-93`). `DraftDocumentCreationPolicyValidator`가 보던 `DraftDictionaryQueryPort`를 걷어냈고 포트·어댑터도 지웠다. **반대 방향은 유효하다** |
| **R-33** | `G-7`·`R-13` 「교정 반영본의 `dictionaryVersionNo`는 **발행 시점 최신 사전집 버전**을 따른다」 | 이 문서 2-2절·4-3절 / `DOMAIN.md` 본문 변경 경로 표 / `DOCUMENT_PLAN.md` 4·6·9절 / `REVIEW_REQUEST_PLAN.md` 9절 | **초안이 대조에 쓴 버전을 따른다**(`D-93`). `DocumentVersionPublishPort`의 시그니처는 그대로고, `ReviseProcessor`가 넘기는 값의 출처만 `ActiveDictionaryVersionQueryPort` → `DraftDocumentSnapshot.dictionaryVersionNo`로 바뀐다. 기준 버전이 없는 옛 초안은 종전 경로를 탄다 |

### 4-7. 룰셋 정족수 상한 정합이 뒤집은 결정 (`R-34`)

| ID | 뒤집힌 결정 | 출처 | 대체 |
| --- | --- | --- | --- |
| **R-34** | 「필수 리뷰어 수의 상한은 **참여자 수 이하**」 | `DOMAIN.md` «워크스페이스 설정 — 리뷰 규칙» 2곳 / `API.md` «워크스페이스 룰셋 수정» / `RuleSet` javadoc·`exceedsParticipantCount` / `WORKSPACE_PLAN.md` 오류 표 / `REVIEW_REQUEST_PLAN.md` 2-1·9절 | **「참여자 수 - 1」 이하**(`D-110`). `DOMAIN.md`·`API.md`와 코드·테스트는 고쳤고, **`docs/plan/`의 두 문서는 보존 원칙에 따라 고치지 않는다** — 그 서술과 어긋나면 이 `R-34`를 따른다. 「상한 검증은 Workspace 책임이고 리뷰 요청 도메인은 참여자 수를 조회하지 않는다」는 부분은 **그대로 유효하다** |

### 4-8. 호출 멱등(워커 선점) 제거가 뒤집은 결정 (`R-35`)

| ID | 뒤집힌 결정 | 출처 | 대체 |
| --- | --- | --- | --- |
| **R-35** | 「워커는 모델을 부르기 전에 `requestId`를 Redis 에 선점하고, 선점 여부를 모르면 그 `REAL` 작업을 실패로 끝낸다」(`D-111`·`D-113`) | `AI_CONTRACT.md` 7-2-2 / `OUTBOX.md` 방어선 그림 / 워커 `app/claim.py`·`tests/test_claim.py` | **선점을 두지 않는다**(2026-09-16). 워커는 재배달을 걸러내지 않고 그대로 처리한다. 코드·의존성(`redis`)·`REDIS_URL` 설정을 모두 제거했다 |

**제거한 이유는 운영에서 이 장치가 작업을 막았기 때문이다.** 워커 컨테이너에 `REDIS_URL`이 없어
**모든 `REAL` 작업이 `CLAIM_UNAVAILABLE`로 실패했고**(2026-09-16, `extractionJobId=1`), 이를 살리려면
ElastiCache 공유·보안그룹·IAM 세 가지를 함께 열어야 했다. 그 비용 대신 장치를 걷어내는 쪽을 택했다.

> ⚠️ **`D-111`이 지목한 위험은 그대로 남는다.** 요청 큐는 표준 큐이고 at-least-once 라(`D-53`)
> 같은 메시지가 두 번 배달되면 **모델을 두 번 부른다.** `D-72`의 콜백 멱등은 초안만 하나로 접으므로
> **DB 는 깨끗하고 요금만 두 배가 되며, 어디에도 실패로 남지 않아 눈에 띄지 않는다.**
> 남아 있는 완화는 둘뿐이다 — `mode=REAL` 메시지에 대해 워커가 모델 호출 전에 Job 상태를 다시 보는
> 것(종결된 작업이면 버린다)과, `D-112`의 `in_progress_flag` 유니크(발행 측 중복만 막는다).
> **둘 다 「이미 진행 중인 같은 작업의 재배달」은 걸러내지 못한다.**
...
`R-8`~`R-14`·`R-19`·`R-32`~`R-33`·`Y-21`이 이 문서들을 향하지만 **보존이 원칙**이다. 각 도메인 세션은 이 문서(`CONFLICTS.md`)를 함께 읽고, 자기 계획 문서의 서술과 여기의 `R-*`가 어긋나면 **`R-*`를 따른다.**

---

## 10. 남은 결정 대기

**없다.** 2026-09-10에 전건 확정했고, 2026-09-12에 드러난 4건은 `D-33`~`D-36`으로, 도메인 Phase 3~4가 드러낸 것은 `D-38`~`D-43`으로 확정했다. 2026-09-13 마무리 통합이 드러낸 3건도 `D-44`~`D-46`으로, Notification 도메인 세션이 확정한 8건은 `D-47`~`D-54`로, RevisionLog 도메인 세션이 확정한 7건은 `D-55`~`D-61`로, 2026-09-14 FE↔BE 통합(트랙 A) 세션이 드러낸 4건은 `D-62`~`D-65`로 확정했다. AI 워커 전환 세션이 확정한 13건은 `D-66`~`D-78`이고, 추출·대조 결과 화면 세션(`T-INT-17`)이 확정한 3건은 `D-79`~`D-81`, 프론트 디자인 정합 세션이 확정한 4건은 `D-82`~`D-85`, 사전집 초안 단일 페이지 전환 세션이 확정한 4건은 `D-86`~`D-89`, 페이징 규격 정합 세션이 확정한 2건은 `D-90`~`D-91`, 문서 삭제 권한 완화 세션이 확정한 1건은 `D-92`, 문서 갱신 상호 배타 완화 세션이 확정한 1건은 `D-93`, 라벨 대소문자 정합 세션이 확정한 1건은 `D-94`, 사전집 개정안 리뷰 UI 수정 세션이 확정한 4건은 `D-95`~`D-98`, 관측성 도입 세션이 확정한 7건은 `D-99`~`D-105`, Flyway 재도입·스키마 정합 세션이 확정한 4건은 `D-106`~`D-109`, 룰셋 정족수 상한 정합 세션이 확정한 1건은 `D-110`, AI 작업 동시성 세션이 확정한 2건은 `D-111`~`D-113`이다.
...
~~**미확정 1건이 생겼다 — `D-111`의 선점 저장소.**~~ **해소** — `D-113`으로 **Redis 로 확정**했다(2026-09-16).
백엔드 claim 엔드포인트 안은 물리쳤다. 워커가 백엔드 DB에 직접 쓰는 방식은 애초에 후보가 아니었다 —
1절의 역할 분담이 워커의 DB 접근을 **읽기로** 한정한다.

**워커 쪽 구현이 남아 있다**(`ubidicExtractor/ubidict-py`). 계약은 `D-113`으로 확정됐지만 코드는 아직 선점하지
않으므로, **그때까지 중복 배달은 그대로 모델을 두 번 부른다.**

새 결정이 필요해지면 루트 `CLAUDE.md`에 따라 **임의로 확정하지 않고 질문한 뒤** 이 문서에 먼저 반영한다. ID는 `D-114`부터 붙인다.
