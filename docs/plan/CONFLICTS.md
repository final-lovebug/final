...
**새 결정 ID는 `D-86`부터** 붙인다. `D-1`~`D-18`을 재사용하지 않는다.
...
새 결정이 필요해지면 루트 `CLAUDE.md`에 따라 **임의로 확정하지 않고 질문한 뒤** 이 문서에 먼저 반영한다. ID는 `D-86`부터 붙인다.
