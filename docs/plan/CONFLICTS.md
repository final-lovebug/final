...
| **D-98** | **관측은 기동 조건이 아니다.** 자격증명·엔드포인트가 없거나 수집기가 죽어 있어도 앱은 뜨고 요청을 처리한다. `${otel.auth-header:}`에 빈 기본값을 두는 것, 수집기가 내려가도 서비스가 계속되는 것을 검증 항목에 두는 것이 이 결정의 표현이다. 또한 **익스포터 자신의 로그를 OTLP로 내보내지 않는다** — export 실패 로그가 다시 export 대상이 되는 되먹임을 끊어야 한다. **끊는 수단은 로거 레벨이 아니라 `logback-spring.xml`의 `additivity="false"`다**(2026-09-15 정정). `HttpExporter`는 연결 실패를 `SEVERE`로, HTTP 상태코드 실패(Grafana Cloud의 401 등)를 `WARNING`으로 찍어서, 레벨을 `ERROR`로 올리면 **시끄러운 쪽은 그대로 나오고 진단에 꼭 필요한 쪽만 가려진다.** 메시지는 stdout에 남겨 둔다 — 텔레메트리가 안 올 때 원인을 볼 유일한 창이고, OTel의 `ThrottlingLogger`가 분당 1회로 묶어 준다 | 관측 도입이 가용성을 낮추지 않게 |
