...
| **Y-07** | `Dictionary`가 「BaseEntity의 deletedAt은 쓰지 않는다. 모든 행이 보존해야 할 버전 이력이라 삭제 경로가 없다」면서 `BaseEntity`를 상속해 `dictionary.deleted_at`이 항상 null로 존재한다 | `Dictionary` javadoc / `V300` 주석 / `AuditableEntity` javadoc("항상 null인 컬럼을 테이블마다 끌고 다니게 된다") | `AuditableEntity`로 바꾸고 `V310`에서 컬럼을 드롭한다 | `DIC-2` |
| **Y-08** | `Term`이 `@CreationTimestamp`/`@UpdateTimestamp`를 직접 선언한다 — `AuditableEntity` 분리 이전에 작성됐다 | `Term` / `AuditableEntity` | `AuditableEntity` 상속으로 대체 | `DIC-2` |
