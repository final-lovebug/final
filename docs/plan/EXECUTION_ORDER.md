...
**결정 ID**는 `D-1`~`D-18`(초안·리뷰 3개 문서 전역 공유), `G-1`~`G-15`(큰 흐름), `D-19`~`D-32`(2026-09-10), `D-33`~`D-37`(2026-09-12 잔여 병렬화), `D-38`~`D-43`(도메인 Phase 3~4), `D-44`~`D-46`(2026-09-13 마무리 통합), `D-47`~`D-54`(2026-09-13 Notification 신설), `D-55`~`D-61`(2026-09-13 RevisionLog 신설), `D-62`~`D-65`(2026-09-14 FE↔BE 통합 트랙 A), `D-66`~`D-78`(2026-09-14 AI 워커 전환), `D-79`~`D-91`(2026-09-14~15 트랙 B 프론트·디자인 정합·사전집 초안·페이징), `D-92`~`D-98`(2026-09-15 관측성 도입)를 쓴다. **새 결정은 `D-99`부터**다 — **원본은 `docs/plan/CONFLICTS.md` 1절이고 이 줄은 사본이다.** 어긋나면 그쪽을 따른다(이 줄이 세 세션 동안 `D-79`에 멈춰 있었다).
...
| **T-INT-6** AI 워커(FastAPI) 전환 | **`DD-5`·`DI-5`** — 둘 다 머지됐다. 추출·대조의 실행을 외부 워커로 옮기고 완료를 HTTP 콜백으로 받는다(`D-66`~`D-78`). **`common/infra/ai/**`·`build.gradle`·`compose.yaml`·`support/**`를 건드리므로 통합 태스크 예외를 쓴다** |
...
| `common/infra/ai/**`, `common/infra/schedule/**` | **`T-INT-6`** | `D-67`·`D-77`. AI 워커 발행 포트와 어댑터, 주기 작업 활성화. **기존 `common` 파일을 고치지 않고 새 패키지를 더한다** — `NT-4`의 `sqs` 패키지와 같은 논거다. 선택 축은 `app.ai.dispatch.mode`이며 `app.messaging.mode`와 독립이다 |
...
> `T-INT-6`은 도메인 대역을 그대로 쓴다 — `V420`(check_job)·`V550`(extraction_job). 통합 태스크지만 두 작업 테이블에 각각 컬럼과 인덱스를 더하는 것이라 공통 대역(900–999)으로 뺄 이유가 없다.

> **대역만으로는 번호 충돌을 막지 못한다(2026-09-14에 실제로 터졌다).** `WLSH-166`(AI 워커 전환)과 `WLSH-171`(후보어 분류)이 **각자 `V550`을 집었고 파일명이 달라 머지가 충돌로 알려 주지 않았다.** 두 파일이 다 들어온 뒤 `flywayInitializer`가 `Found more than one migration with version 550`으로 기동을 막았다. 나중 것을 `V560__add_candidate_term_type.sql`로 옮겨 해소했다(대역·10 단위 규칙은 그대로).
>
> **같은 도메인 대역을 두 태스크가 동시에 건드리면 번호를 먼저 맞춘다.** 대역 분리는 *도메인 간* 충돌만 막는다 — 한 대역 안에서 나란히 진행하는 태스크는 서로의 마지막 번호를 확인해야 한다.
>
> **번호를 옮길 때는 「이미 적용된 쪽」을 건드리지 않는다.** 적용된 것을 옮기면 「적용됐는데 로컬에 없는 마이그레이션」이 되어 더 나빠진다 — 옮길 것은 아직 적용되지 않은 쪽이다. 그리고 **새 번호가 이미 적용된 최신 버전보다 낮으면 다음 기동이 `Detected resolved migration not applied to database`로 막힌다.** 그때는 **개발 DB를 리셋한다** — `T-INT-1`(out-of-order)을 폐기한 근거가 정확히 「항상 리셋하므로 순서 역전을 감당할 필요가 없다」였다. `compose.yaml`의 MySQL에는 명명된 볼륨이 없어 `docker rm -f final-mysql-1` 뒤 `bootRun`이면 새로 만들어진다.
...
>
> **`NT-4`의 `common/infra/event/sqs/` 추가도 예외다**(`D-52`). `ARCHITECTURE.md` «이벤트 발행 규약 — 배치»가 배포용 어댑터를 그 자리에 두라고 명시한다. 새 패키지를 더하는 것이라 기존 파일과 충돌하지 않는다.
