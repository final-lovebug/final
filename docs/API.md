...
- **수신 설정과 채널은 MVP1에 없다**(`D-51`). 전달 수단이 인앱 하나뿐이라 고를 것이 없다. 응답에 `channels`가 없는 이유이기도 하다. Slack·메일은 `REQ-NTF-005`~`006`으로 MVP2다.
...
**버튼 문구와 이동 경로는 응답에 없다.** `type`과 `targetType`·`targetId`에서 화면이 파생한다(`D-46`).
