...
**예외 — 내부 워커 콜백.** `/api/internal/**`은 인증 주체가 없다. AI 워커(FastAPI)가 작업 결과를 돌려주는 서버-투-서버 경로이며 회원 principal이 존재하지 않는다. 대신 **작업마다 발행되는 1회용 상관 식별자(`requestId`)를 본문으로 받아 작업 행에 저장된 값과 대조**하고, 배포 환경에서는 네트워크로 출발지를 제한한다(`NFR-AI-002`·`D-70`). **이 경로도 요청자를 본문으로 받지 않는다** — 행위의 주체는 작업 행에 이미 기록된 `requestedBy`이므로 「요청자를 클라이언트가 참칭하지 못한다」는 원칙의 취지는 그대로다. 그 밖의 `/api/**`에는 예외가 없다.
...
작업을 `PENDING`으로 저장하고 대조 요청 이벤트를 발행한 뒤 `202 Accepted`와 작업 상태를 즉시 반환한다. 같은 문서에 `PENDING` 또는 `RUNNING` 작업이 있으면 중복 접수를 거절한다. 이 검사는 조회만으로 끝나지 않는다 — DB가 **문서당 진행 중 작업 1개**를 최종 보장하므로, 동시에 요청해도 한 건만 AI 요청으로 이어지고 나머지는 같은 `409 DRAFT_DOCUMENT_CHECK_ALREADY_RUNNING`으로 끝난다.
...
`POST /api/draft-dictionaries/extractions`는 용어 추출 작업을 비동기로 접수하고 `202 Accepted`를 응답한다. 워크스페이스 관리자 이상만 요청할 수 있으며, 전달한 문서 중 현재 활성 사전집 기준과 정렬되고 직접 편집되지 않은 문서만 작업 대상에 남긴다. 대상 문서가 하나도 없거나 같은 워크스페이스에 진행 중인 초안·추출 작업이 있으면 요청을 거절한다. 특히 추출은 AI 모델 호출을 일으키므로, DB가 **워크스페이스당 진행 중 작업 1개**를 최종 보장한다. 동시 접수 중 한 건만 AI 요청으로 이어지고 나머지는 `409 DRAFT_DICTIONARY_EXTRACTION_ALREADY_RUNNING`이다.
...
`GET /api/workspaces/{workspaceId}/notifications?page=0&size=20&unreadOnly=false`
