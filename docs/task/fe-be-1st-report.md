...
| T-INT-14 member | 🔴 보류 → **A 해결 완료, 지금 재개 가능**(`T-INT-18` 완료됨) |
...
1. ~~`T-INT-18` 착수~~ — ✅ **완료**(2026-09-14). `T-INT-14`는 지금 재개 가능.
...
6. 위가 정리되는 대로 트랙 A 잔여(T-INT-10 나머지 4개, T-INT-11 후보어, T-INT-14)
   마저 진행.
7. 완료된 부분(T-INT-9·11(부분)·10(부분)·15·16·18)은 로컬 백엔드로 QA 후 커밋/PR —
   사용자 지시 시 진행.
