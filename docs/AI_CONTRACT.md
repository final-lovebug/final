...
| **호출 멱등**(7-2-2) | 워커 | 같은 작업 메시지로 모델을 두 번 부르는 것 | — |
...
따라서 같은 추출 또는 대조 버튼을 동시에 여러 번 눌러도 **접수 경로에서 AI 요청은 하나만 만들어진다.** 이 보장은 작업 행 생성까지이며, 큐 재배달 이후의 모델 호출 중복은 7-2-2의 워커 선점으로 막는다.
...
#### 7-2-2. 호출 멱등 — 워커가 보장해야 한다 (`D-111`)

요청 큐는 표준 큐이고 at-least-once 다(4-1). **같은 메시지가 두 번 배달되면 워커는 같은 작업으로 모델을 두 번 부른다.**

**`requestId`가 멱등 키다**(`D-70`). 워커는 **모델을 부르기 전에** 이 키를 선점하고, 선점에 실패하면 그 메시지로는 모델을 부르지 않는다.
...
**키를 지우지 않는다.** 작업이 끝나도 TTL 이 만료될 때까지 남겨 둔다 — 지우면 그 뒤에 도착한 재배달분이 다시 선점에 성공해 모델을 또 부른다. `requestId` 는 작업 하나에만 쓰이는 1회용 값이라(`D-70`) 키가 남아도 다음 작업을 막지 않는다.

**`REDIS_URL`이 없거나 Redis 가 죽으면 그 `REAL` 작업을 실패로 끝낸다.** 선점 여부를 모르는 채로 모델을 부르면 이 절이 막으려던 바로 그 일이 일어난다. 워커는 실패 콜백(6-3)을 보내고, 사용자는 폴링에서 사유를 보고 다시 요청한다. **조용히 선점을 건너뛰지 않는다** — 건너뛰면 이 절이 무의미해진다.

> **백엔드의 Redis 와 같은 인스턴스일 필요는 없다.** 백엔드는 이 키를 읽지도 쓰지도 않는다 — 선점은 전적으로 워커 안에서 끝난다.
