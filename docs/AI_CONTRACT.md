...
이미 나간 호출을 되돌리지 못한다. 재배달로 모델을 두 번 부르는 것은 **막지 않는다** — 7-2-2 참고.
...
### 7-2. 중복 방지와 멱등 — 셋으로 나뉜다

**보장하는 시점과 주체가 다르고, 막는 것도 다르다.** 접수 배타는 사용자가 같은 작업을 동시에 접수해 AI 요청이 여러 건 만들어지는 것을 막는다. 하지만 표준 SQS의 재배달로 이미 발행된 메시지가 중복되는 경우까지 막지는 못하므로, 워커의 호출 멱등도 별도로 필요하다.

| | 보장 주체 | 막는 것 | 막지 못하는 것 |
| --- | --- | --- | --- |
| **접수 배타**(7-2-0) | 백엔드 DB | 동시 사용자 요청으로 작업·AI 요청이 여러 건 생기는 것 | 이미 발행된 SQS 메시지의 재배달 |
| **결과 멱등**(7-2-1) | 백엔드 | 같은 작업의 콜백으로 초안·작업 상태가 두 벌 생기는 것 | **이미 나간 모델 호출** |
| ~~**호출 멱등**(7-2-2)~~ | — | **막지 않는다**(`R-35`). 중복 배달마다 모델이 한 번 더 불린다 | — |

#### 7-2-0. 접수 배타 — 백엔드 DB가 보장한다

접수 전의 `PENDING`·`RUNNING` 존재 조회는 빠른 409 응답을 위한 사전 검사일 뿐, 동시 트랜잭션을 막는 최종 수단이 아니다. 두 요청이 같은 스냅샷을 읽으면 둘 다 사전 검사를 통과할 수 있다.

그래서 작업 저장 시 DB 유니크 제약이 최종 판정한다. 제약 충돌은 순차 요청과 같은 `409`으로 변환하고, 실패한 요청은 작업 행도 내부 발행 신호도 남기지 않는다.

- `TERM_EXTRACTION`: 삭제되지 않은 `PENDING` 또는 `RUNNING` 작업은 **워크스페이스당 1개**만 허용한다.
- `DOCUMENT_CHECK`: 삭제되지 않은 `PENDING` 또는 `RUNNING` 작업은 **문서당 1개**만 허용한다.
- `SUCCEEDED`·`FAILED` 작업은 진행 중이 아니므로 다음 접수를 막지 않는다.

따라서 같은 추출 또는 대조 버튼을 동시에 여러 번 눌러도 **접수 경로에서 AI 요청은 하나만 만들어진다.** 이 보장은 작업 행 생성까지다. **큐 재배달 이후의 모델 호출 중복은 막지 않는다**(7-2-2).

#### 7-2-1. 결과 멱등 — 백엔드가 보장한다 (`D-72`)

같은 메시지를 두 번 받아 콜백을 두 번 보내도 **초안은 하나만 생긴다.** 이미 끝난 작업에 도착한 콜백은 아무것도 바꾸지 않고 **204**로 답한다 — 성공한 작업에 늦게 도착한 실패 콜백도 성공을 뒤집지 않는다.

그러므로 **워커는 콜백 전달에 확신이 없으면 다시 보내도 된다.** 다만 4xx는 재시도하지 않는다.

**이것은 DB 상태의 멱등이다.** 중복 배달로 모델을 두 번 불렀다면 그 호출은 이미 끝났고 요금도 이미 나갔다. 뒤에서 초안이 하나로 접히므로 **DB는 깨끗하고 비용만 두 배가 된다** — 어디에도 실패로 남지 않아 눈에 띄지 않는 종류의 낭비다.

#### 7-2-2. 호출 멱등 — 두지 않는다 (`R-35`, 종전 `D-111`·`D-113`)

요청 큐는 표준 큐이고 at-least-once 다(4-1). **같은 메시지가 두 번 배달되면 워커는 같은 작업으로 모델을 두 번 부른다.**

**그것을 막는 장치를 두지 않기로 했다**(2026-09-16). 종전 규격은 워커가 모델 호출 직전에 `requestId`를 Redis 에 선점하고, 선점 여부를 모르면 그 `REAL` 작업을 실패로 끝내는 것이었다(`D-111`·`D-113`). 운영에서 워커 컨테이너에 `REDIS_URL`이 없어 **모든 `REAL` 작업이 `CLAIM_UNAVAILABLE`로 실패했고**, 되살리는 비용(ElastiCache 공유 + 보안그룹 + IAM) 대신 장치를 걷어냈다. 배경은 `docs/plan/CONFLICTS.md` `R-35`.

> ⚠️ **위험은 남아 있다.** 중복 배달마다 모델이 한 번 더 불린다. 7-2-1 의 결과 멱등이 **초안만** 하나로 접으므로 **DB 는 깨끗하고 요금만 두 배가 되며, 어디에도 실패로 남지 않아 눈에 띄지 않는다.** 요금이 문제가 되면 이 절을 되살리는 것이 첫 후보다.

지금 남아 있는 완화는 둘이고, **둘 다 「진행 중인 같은 작업의 재배달」은 걸러내지 못한다.**

| 완화 | 무엇을 막나 | 못 막는 것 |
| --- | --- | --- |
| 워커가 `mode=REAL`에서 모델 호출 전 Job 상태를 다시 본다 | 타임아웃·DLQ 가 이미 회수한 **종결된** 작업의 지연 메시지 | 아직 `RUNNING`인 작업의 재배달 — 상태가 정상이라 그대로 처리한다 |
| `in_progress_flag` 유니크(`D-112`) | **발행 측** 중복 — 작업 자체가 둘 생기는 것 | 큐의 재배달. 작업은 하나이고 메시지만 둘이다 |

**백엔드가 대신해 줄 수 없다.** 메시지를 받는 쪽이 워커이고 「모델을 부르기 직전」이라는 시점은 워커 안에만 있다. 백엔드의 `PENDING → RUNNING` 전이는 **발행 측 중복만** 막고 큐의 재배달에는 닿지 않는다.

> **FIFO 큐의 `MessageDeduplicationId`로 대신할 수도 없다.** 중복 제거 창이 5분이라 `app.ai.timeout.job`(prod 기본 50분)보다 짧아 긴 작업을 덮지 못하고, 큐 종류를 바꾸는 것은 `D-53`을 뒤집는 일이다.
...
| 1 | 2026-09-16 | 7-2 를 **결과 멱등(백엔드)과 호출 멱등(워커)으로 나눴다**(`D-111`). 종전 서술은 「확신이 없으면 다시 보내도 된다」로 끝나 워커 입장에서 **모델 재호출도 안전하다고 읽혔다** — 실제로는 중복 배달마다 요금이 두 배로 나간다 | 호환 — 메시지 스키마는 그대로고 **워커에 선점 의무가 추가된다** |
| 1 | 2026-09-16 | 호출 멱등의 선점 저장소를 **Redis 로 확정**했다(`D-113`). 키·TTL·장애 시 동작은 7-2-2 | 호환 — **워커에 `REDIS_URL` 의존성이 추가된다** |
| 1 | 2026-09-16 | **호출 멱등을 제거했다**(`R-35`). 위 두 줄(`D-111`·`D-113`)을 뒤집는다 — 운영에서 `REDIS_URL` 부재로 모든 `REAL` 작업이 실패해, 되살리는 비용 대신 장치를 걷어냈다 | 호환 — **워커의 `REDIS_URL` 의존성이 사라진다.** 중복 배달 시 모델 재호출이 열린다 |
