...
- ~~**`ErrorResponse`에 `traceId`가 없다.**~~ **해소** — `T-INT-23`(관측성 도입)이 담당을 이어받아 `ErrorResponse`에 `traceId`를 더했다(`X-08`·`D-96`). 값은 Micrometer Tracing이 MDC에 넣는 것을 쓰고 별도 필터를 만들지 않는다. **`NFR-INF-008`(Spring → 메시징 → FastAPI 3단 추적)은 여전히 미충족이다** — 앱 구간까지만 이어졌고 워커 구간 `traceparent` 전파가 남았다.
