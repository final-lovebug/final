...
public class TestcontainersConfiguration {
     ...
     *
     * <p>필요한 서비스만 올리고 즉시 로딩해 기동 시간을 줄인다. {@code withReuse}는 켜지 않는다 — 머신마다 opt-in이라 CI와 로컬이 갈리고, 무엇보다
     * 큐가 실행 사이에 살아남아 지난 실행의 메시지가 다음 실행의 첫 테스트를 때린다.
     *
     * <p>{@code ssm}은 관측 자격증명 경로를 검증하려고 켠다(T-INT-23). {@code /lovebug/otel/auth}가 {@code otel.auth} 프로퍼티로
     * 매핑되고 그 뒤에 {@code OtlpAuthHeaderEnvironmentPostProcessor}가 도는지는 Parameter Store 를 실제로 읽어야만 확인할 수 있다.
     */
    @Bean
    LocalStackContainer localStackContainer() {
        return new LocalStackContainer(DockerImageName.parse("localstack/localstack:4"))
                .withEnv("SERVICES", "sqs,ssm")
