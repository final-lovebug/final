 ...
 *
 * <p><b>{@code EventEnvelope}를 쓰지 않는 이유</b>(D-67·R-28) — 봉투는 {@code eventType}에 Java 클래스의 단순 이름을 싣고 본문은
 ...
 *
 * <p><b>본문을 싣지 않는 이유</b>(D-69) — 문서 본문 상한이 10,000자라 여러 건을 담으면 SQS 표준 메시지 한도 256KB를 넘는다. 워커는 식별자만 받고
 ...
 *
 * @param requestId 발행마다 새로 만드는 UUIDv4. <b>콜백 인증 토큰을 겸한다</b>(D-70) — 워커는 결과를 돌려줄 때 이 값을 그대로 실어야 하고,
