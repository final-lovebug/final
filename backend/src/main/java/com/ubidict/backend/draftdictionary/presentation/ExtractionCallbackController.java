...
/**
 * AI 워커(FastAPI)가 추출 결과를 돌려주는 자리(D-62).
 *
 * <p><b>인증 주체가 없는 유일한 업무 API다.</b> {@code /api/internal/**}은 필터 체인을 그냥 통과하며(D-65), 호출자 확인은 본문의
 * {@code requestId}를 작업 행에 저장된 값과 대조해서 한다(D-66). 그래서 {@code @AuthenticationPrincipal}을 받지 않는다 — 행위의 주체는
 ...
 *
 * <p>응답 코드가 워커의 재시도 여부를 정한다(D-69) — 2xx와 4xx는 메시지를 지우고, 5xx와 타임아웃만 재시도한다. 이미 끝난 작업에 도착한 중복·지각 콜백도
 * <b>204</b>다(D-68). 4xx로 답하면 워커가 영원히 재시도한다.
 */
@RestController
